...
def bl602_demo_event_ble(env, extra_data):
    ...
    try:
        ...
        dut1.expect(re.compile(r'Confirm pairing for ((\w{2}.){6})\((\w{6})\)'), timeout=0.5)
        dut1.write('ble_auth_pairing_confirm')
        print('[DUT1] Auth pairing confirm')

        dut1.expect(re.compile(r'Bonded with ((\w{2}.){6})\((\w{6})\)'), timeout=1)
        print('[DUT1] Bonded')
        dut2.expect(re.compile(r'Bonded with ' + str(a) + '\((\w{6})\)'), timeout=1)
        print('[DUT2] Bonded')

        dut2.write('ble_exchange_mtu')
        mtu = dut2.expect(re.compile(r'Exchange successful MTU Size =(.*)'), timeout=0.5)[0]
        print('[DUT2] Mtu size = ', mtu)

        # Discover
        dut2.write('ble_discover 0 0 0x1 0xffff')
        dut2.expect(re.compile(r'Service ((\w{4})) found: start handle \w, end_handle \w'), timeout=0.5)
        time.sleep(0.5)
        print('[DUT2] Ble discover primary service')

        dut2.write('ble_discover 1 0 0x1 0xffff')
        dut2.expect('Discover complete', timeout=0.5)
        time.sleep(0.5)
        ...
        dut2.expect(re.compile(r'Characteristic ((\w{4})) found: attr->handle \w  chrc->handle \w'), timeout=0.5)
        time.sleep(0.5)
        print('[DUT2] Ble discover characteristic')

        dut2.write('ble_discover 4 0 0x1 0xffff')
        descriptor = dut2.expect(re.compile(r'Descriptor (.*) found: handle \w'), timeout=0.5)[0]
        time.sleep(0.5)
