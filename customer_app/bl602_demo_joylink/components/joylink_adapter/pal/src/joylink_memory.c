...
#endif
#endif
    /* log_info("[PAL MALLOC] %lu B\r\n", size); */
    void *ret = pvPortMalloc(size);
    if (ret == NULL) {
        log_error("[PAL MALLOC] FATAL: failed\r\n");
    }
    return ret;
}
...
#endif
    void *ret = NULL;
    /* log_info("[PAL REALLOC] %lu B\r\n", size); */
    if (ptr == NULL) {
        ret = jl_platform_malloc(size);
        goto ret;
    }

    /* ptr is not NULL and size is 0 */
    if (size == 0) {
        jl_platform_free(ptr);
        goto ret;
    }
    /* ptr is not NULL and size is not 0 */
    ret = jl_platform_malloc(size);
    if (ret) {
        memcpy(ret, ptr, size);
        jl_platform_free(ptr);
