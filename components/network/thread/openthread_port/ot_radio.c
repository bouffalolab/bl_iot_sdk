...
#if OPENTHREAD_CONFIG_TIME_SYNC_ENABLE || OPENTHREAD_CONFIG_MAC_CSL_RECEIVER_ENABLE || OPENTHREAD_CONFIG_MAC_CSL_TRANSMITTER_ENABLE
        p->frame.mInfo.mRxInfo.mTimestamp = otRadio_var.tsIsr - OT_RADIO_SYMBOL_TIME * (p->frame.mLength + 2 + 1) * 2;
#else
        p->frame.mInfo.mRxInfo.mTimestamp = otRadio_var.tsIsr;
#endif
