...
uint32_t usbd_video_mjpeg_payload_fill(uint8_t *input, uint32_t input_len, uint8_t *output, uint32_t *out_len)
{
    ...
    packet_size = (usbd_video_cfg.probe.dwMaxPayloadTransferSize - 2);
    packets = input_len / packet_size;

    if (input_len % packet_size) {
        packets++;
    }
    last_packet_size = input_len - (packets - 1) * packet_size;
    ...
    for (uint32_t i = 0; i < packets; i++) {
        ...
        if (i == (packets - 1)) {
            memcpy(&output[2 + usbd_video_cfg.probe.dwMaxPayloadTransferSize * i], &input[picture_pos], last_packet_size);
        } else {
            memcpy(&output[2 + usbd_video_cfg.probe.dwMaxPayloadTransferSize * i], &input[picture_pos], packet_size);
            picture_pos += packet_size;
        }
    }
...
void usbd_video_mjpeg_payload_header_toggle(uint8_t *output, uint32_t packets)
{
    for (size_t i = 0; i < packets; i++) {
        output[usbd_video_cfg.probe.dwMaxPayloadTransferSize * i + 1] ^= 1;
    }
}
